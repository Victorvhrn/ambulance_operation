...
Data::Data(Solver* solver, Call* call, Ambulance* ambulance): solver(solver),
	ins(solver->ins), travel(solver->travel), call(call), ambulance(ambulance){
	...
	num_times = 12;
	// quantum = (num_times*1800-solver->time)/(num_times-2);
	quantum = (solver->last_time - solver->time)/(num_times-2);
	...
	for(int c = 0; c < types_call; ++c){
		...
		for(int l = 0; l < num_locals; ++l){
			...
			for(int h = 0; h < num_hosps; ++h){
				double dist = travel.norm(ins.centers[l], ins.hospitals[h]);
				dists.push_back(std::make_pair(dist,h));
			}
			std::sort(dists.begin(), dists.end());
			for(int k = 0; k < std::min(g_params.n_nearest_hospitals,num_hosps); ++k)
	...
	for(int i = 0; i < solver->queue.size(); ++i){
		auto future_call = solver->calls[solver->queue[i]];
		if(call == NULL  || future_call != *call){
			int location_index = -1;
			location_index = get_location_index(future_call.location);
	...
	for(auto& amb: solver->ambulances){
		if(amb.arrival_time_at_f_last_trip > solver->time + g_params.EPS){
			Location origin_location = amb.last_origin_location;
			Location current_location = travel.position_between_origin_destination(
				origin_location, amb.call->location, amb.departure_time,
				solver->time);
			double travel_time = travel.travel_time(origin_location,
				amb.call->location);
			if(current_location == amb.call->location){
				current_location =  travel.position_between_origin_destination(
					amb.call->location, solver->ins.hospitals[amb.call->hospital],
					amb.departure_time + travel_time, solver->time);
				int l1 = get_location_index(current_location);
				A0_calh[amb.call->priority][amb.type][l1][amb.call->hospital] += 1;
			}else{
				int l1 = get_location_index(current_location);
				int l = get_location_index(amb.call->location);
				int h = amb.call->hospital;
				A0_callh[amb.call->priority][amb.type][l1][l][h]+=1;
			}
		}else{
			if(amb.arrival_time_at_b_last_trip <= solver->time){
				int base_ind = -1;
				for(int b = 0; b < num_bases; ++b){
					if(travel.norm(amb.base_location, ins.bases[b]) < g_params.EPS){
						base_ind = b;
					}
				}
				A0_ab[amb.type][base_ind] += 1;
			}else if(abs(solver->time - amb.arrival_time_at_f_last_trip) > 
				g_params.EPS){
				Location current_location = travel.ambulance_position(amb, 
					solver->time);
				...
				for(int h = 0; h < num_bases; ++h){
					if(travel.norm(amb.free_location, ins.hospitals[h]) < g_params.EPS){
						hosp_ind = h;
					}
				}
	...
	for(int i = 1; i < solver->calls.size(); ++i){
		auto& call = solver->calls[i];
			...
			for(int a = 0; a < types_amb; ++a){
				...
				for(int l1 = 0; l1 < total_locals; ++l1){
					...
					for(int l = 0; l < num_locals; ++l){
						...
						for(int h = 0; h < num_hosps; ++h){
							tao[t][c][a][l1][l][h] = travel.travel_time(locations[l1], 
								locations[l]) + travel.travel_time(locations[l],
								ins.hospitals[h]);
	...
	for(int c = 0; c < types_call; ++c){
		...
		for(int a = 0; a < types_amb; ++a){
			...
			for(int l1 = 0; l1 < total_locals; ++l1){
				...
				for(int h = 0; h < num_hosps; ++h){
					tao_0[c][a][l1][h] = travel.travel_time(locations[l1], 
						ins.hospitals[h]);
				}
			}
		}
...
int Data::L(int t, int a, int l1, int b){
	...
	double time_l1_b = travel.travel_time(locations[l1], base);
	if(time_l1_b <= quantum){
		return (b < num_bases) ? bases[b] : b;
	}else{
		Location current_location = travel.position_between_origin_destination(
			locations[l1], base, solver->time + t*quantum, 
			solver->time + (t+1)*quantum);
		return get_location_index(current_location);
	}
}

int Data::get_time_slot(double time){
	if(time <= solver->time){
		return -1;
	}
	int t = (int) ((time-solver->time)/quantum);	
	return t;
}
...
int Data::get_location_index(Location& location){
	// for(int b = 0; b < num_bases; ++b){
	// 	if(travel.norm(location, ins.bases[b]) < g_params.EPS){
	// 		return bases[b];
	// 	}
	// }

	// for(int h = 0; h < num_hosps; ++h){
	// 	if(travel.norm(location, ins.hospitals[h]) < g_params.EPS){
	// 		return hospitals[h];
	// 	}
	// }
	...
	double ind_l = -1;
	for(int l = 0; l < num_locals; ++l){
		double dist_to_region =  travel.norm(location, ins.centers[l]);
...
Data::~Data(){
	...
	delete[] H; delete[] A0_alb; delete[] A0_calh; delete[] A0_callh;
	delete[] lambda; delete[] tao; delete[] tao_0;
	delete[] C; delete[] A0_ab; delete[] A0_ah;
	delete[] hospitals; delete[] bases; delete[] is_hospital;
	delete[] is_base;
}


// Data::Data(EventSimulator& sim, Call* call, Ambulance* ambulance): ins(sim.ins), sim(sim), 
// 	debug(g_params.debug), call(call), ambulance(ambulance){
...
// 			std::vector<std::pair<double, int>> dists;
// 			for(int h = 0; h < num_hosps; ++h){
// 				double dist = sim.norm(ins.hospitals[h], locations[l]);
// 				dists.push_back(std::make_pair(dist,h));
// 			}
// 			std::sort(dists.begin(), dists.end());
// 			for(int k = 0; k < std::min(g_params.n_nearest_hospitals,num_hosps); ++k)
...
// 			int location_index = -1;
// 			location_index = get_location_index(call_queue->location);
...
// 			double time_to_call = sim.travel_time(amb.trips.back(), amb.call->location, 
// 				amb.speed);
// 			std::cout << sim.show_location(current_location) << ", index = ";
// 			std::cout << location_index << ". Going to: "; 
// 			if(departure_time + time_to_call <= sim.time){
// 				std::cout << "hosp " << sim.show_location(amb.hospital_location) << ", from ";
// 				std::cout << sim.show_location(amb.last_origin_location) << ", ";
// 				std::cout << "h = " << hosp_ind << "\n";
// 				A0_calh[amb.call->priority][amb.type][location_index][hosp_ind] += 1;
// 			}else{
// 				int call_location_index = get_location_index(amb.call->location);
...
// 		int hosp_ind = -1;
// 		for(int h = 0; h < num_hosps; ++h){
// 			if(sim.norm(ambulance->hospital_location, sim.ins.hospitals[h]) < EPS){
// 				hosp_ind = h;
// 				break;
// 			}
// 		}
// 		if(hosp_ind != -1){
// 			std::cout << "Amb " << ambulance->id << " t" << ambulance->type << "at h = ";
// 			std::cout << hosp_ind << "\n";
// 			A0_ah[ambulance->type][hosp_ind] += 1;
// 			amb_nodes_map[ambulance->id].second = hosp_ind;
// 		}else{
// 			std::cout << "ERROR: ambulance is not at any hospital\n";
// 			exit(1);
// 		}
// 	}
...
// 	// lt_tab
	...
	// 	int t = get_time_slot(call.time);
	// 	if(t != -1){
	// 		point_t bg_call_location(call.location.first, call.location.second);
	// 		int l_index = -1;
	// 		// std::cout << "location: "<< call.location.first << " " << call.location.second << "\n";
	// 		for(int l = 0; l < num_locals; ++l){
	// 			if(bg::covered_by(bg_call_location,regions[l])){
	// 				l_index = l;
	// 				break;
	// 			}
	// 		}
...
// 		for(int j = 0; j < L; ++j){
// 			polygon_t poly;
// 			bg::append(poly.outer(), point_t(x_left,y_bottom_origin));
// 			bg::append(poly.outer(), point_t(x_left,y_top_origin));
// 			bg::append(poly.outer(), point_t(x_right,y_top_origin));
// 			bg::append(poly.outer(), point_t(x_right,y_bottom_origin));
// 			bg::append(poly.outer(), point_t(x_left,y_bottom_origin));
...
// 			// 	it != boost::end(bg::exterior_ring(poly)); ++it){
