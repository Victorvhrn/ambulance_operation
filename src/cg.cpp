...
int get_time(Data& data, int time){
	return 1+ (int)time/data.quantum;
}

CGCall::CGCall(Data& data, GRBEnv& env, Solver& solver): data(data), env(env), model(env), 
	solver(solver), columns_added(0), type_call0(data.type_call0), local_call0(data.local_call0){
	...
	for(int l = 0; l < data.num_locals; ++l){
		...
		for(int l1 = 0; l1 < data.num_locals; ++l1){
			auto dist_l1_l = travel.norm(data.locations[l1], data.locations[l]);
	...
	for(int a = 0; a < data.types_amb; ++a){
		...
		for(int b = 0; b < data.num_bases; ++b){
			...
			for(int h = 0; h < data.num_hosps; ++h){
				if(data.A[type_call0].find(a) != data.A[type_call0].end() &&
					...
					x0_abh[a][b][h] = model.addVar(0, A0_ab[a][b], 0, GRB_CONTINUOUS, 
						name.str());
					name.str("");	
				}
		...
		for(int h1 = 0; h1 < data.num_hosps; ++h1){
			...
			for(int h = 0; h < data.num_hosps; ++h){
				if(data.A[type_call0].find(a) != data.A[type_call0].end() &&
					...
					x0_ahh[a][h1][h] = model.addVar(0, A0_ah[a][h1], 0, GRB_CONTINUOUS,
						name.str());
					name.str("");
				}
		...
		for(int h = 0; h < data.num_hosps; ++h){
			...
			for(int  b = 0; b < data.num_bases; ++b){
				...
				y0_ahb[a][h][b] = model.addVar(0,GRB_INFINITY, 0, GRB_CONTINUOUS, name.str());
				name.str("");
			}
		}
	}
		...
		for(int a = 0; a < data.types_amb; ++a){
			...
			for(int h = 0; h < data.num_hosps; ++h){
				...
				for(int b = 0; b < data.num_bases; ++b){
					...
					yt_ahb[t][a][h][b] = model.addVar(0,GRB_INFINITY,0,
						GRB_CONTINUOUS, name.str());
					name.str("");
				}
			}
		}
	...
	for(int t = 0; t < data.num_times; ++ t){
		...
		for(int a = 0; a < data.types_amb; ++a){
			...
			for(int b = 0; b < data.num_bases; ++b){
				...
				At_ab[t][a][b] = model.addVar(0,GRB_INFINITY, 0,
					GRB_CONTINUOUS, name.str());
				name.str("");
			}
		}
			...
			for(int l = 0; l < data.total_locals; ++l){
				...
				for(int b = 0; b < data.num_bases; ++b){
					...
					if(data.L_tab[t][a][b].find(l) !=
						...
						name << "_b" << b;
						At_alb[t][a][l][b] = model.addVar(0,GRB_INFINITY, 0, 
							GRB_CONTINUOUS, name.str());
						name.str("");	
					}
				}
					...
					for(int l = 0; l < data.num_locals; ++l){
						for(int h = 0; h < data.num_hosps; ++h){
							if(data.H[c][l].find(h) != data.H[c][l].end()){
								...
								for(int h1 = 0; h1 < data.num_hosps; ++h1){
									fo += f(t,c,a,data.hospitals[h1],l,
										h,data,factor)*
	...
	for(int t = 0; t < data.num_times; ++t){
		for(int c = 0; c < data.types_call; ++c){
			factor = data.ins.penalties[c]*3600;
			for(int l = 0; l < data.num_locals; ++l){
...
void CGCall::solve(){
	...
	do{
		...
		std::chrono::high_resolution_clock::time_point dt_dual = 
			std::chrono::high_resolution_clock::now();
		sum_dual_time += std::chrono::duration_cast<std::chrono::nanoseconds>
			(dt_dual - t_dual).count() / pow(10,9);
		std::chrono::high_resolution_clock::time_point t0 = 
			std::chrono::high_resolution_clock::now();
		column_found = pricing();
		std::chrono::high_resolution_clock::time_point dt = 
			std::chrono::high_resolution_clock::now();
		sum_pricing_time += std::chrono::duration_cast<std::chrono::nanoseconds>
	...
	if(model.get(GRB_IntAttr_Status) != 2){
		...
		// std::cout << "Z (GC): " << model.get(GRB_DoubleAttr_ObjVal) << "\n";	
	}
...
void CGCall::set_dual(){
	...
	// 				psi0_alb[a][l1][b] = con_location_t0[a][l1][b].get(GRB_DoubleAttr_Pi);
	// 				// if(abs(psi0_alb[a][l1][b]) > g_params.g_params.EPS){
	// 				// 	std::cout << "psi0_" << a << "_" << l1 << "_" << b << ": ";
	...
	for(int t = 0; t < data.num_times-1; ++ t){
		for(int a = 0; a < data.types_amb; ++a){
			for(int b = 0; b < data.num_bases; ++b){
				// if(abs(beta_tab[t][a][b]-con_base[t][a][b].get(GRB_DoubleAttr_Pi)) > g_params.g_params.EPS){
				// 	arq << "beta_" << t+1 << "_" << a << "_" << b << ",";
				// 	arq << beta_tab[t][a][b] << "," << con_base[t][a][b].get(GRB_DoubleAttr_Pi);
				// 	arq << "\n";
				// }
				beta_tab[t][a][b] = con_base[t][a][b].get(GRB_DoubleAttr_Pi);
				gamma_tab[t][a][b] = con_amb_base[t][a][b].get(GRB_DoubleAttr_Pi);
				// if(iter >= 4  && abs(beta_tab[t][a][b]) > g_params.EPS){
				// 	std::cout << "beta_" << t+1 << "_" << a << "_" << b << ",";
				// 	std::cout << beta_tab[t][a][b] << "\n";
				// }
			...
			for(int h = 0; h < data.num_hosps; ++h){
				// if(abs(alpha_tah[t][a][h]-con_hospital[t][a][h].get(GRB_DoubleAttr_Pi)) > g_params.EPS){
				// 	arq << "alpha_" << t+1 << "_" << a << "_" << h << ",";
				// 	arq << alpha_tah[t][a][h] << ",";
				// 	arq << con_hospital[t][a][h].get(GRB_DoubleAttr_Pi) << "\n";
				// }
				alpha_tah[t][a][h] = con_hospital[t][a][h].get(GRB_DoubleAttr_Pi);
				// if(iter >= 4 && abs(alpha_tah[t][a][h]) > g_params.EPS){
			...
			for(int l1 = 0; l1 < data.num_locals; ++l1){
				for(int b = 0; b < data.num_bases; ++b){
					if(data.L_tab[1][a][b].find(l1) != data.L_tab[1][a][b].end()){
						// if(abs(psi_talb[t][a][l1][b]-con_location[t][a][l1][b].get(
						// 	GRB_DoubleAttr_Pi)) > g_params.EPS){
						// 	arq << "psi_" << t+1 << "_" << a << "_" << l1 << "_";
						// 	arq << b << "," << psi_talb[t][a][l1][b] << ",";
						// 	arq << 	con_location[t][a][l1][b].get(GRB_DoubleAttr_Pi);
						// 	arq << "\n";
						// }
						psi_talb[t][a][l1][b] = con_location[t][a][l1][b].get(
							GRB_DoubleAttr_Pi);

						// if(iter >= 4 && abs(psi_talb[t][a][l1][b]) > g_params.EPS){
						// 	arq << "psi_" << t+1 << "_" << a << "_" << l1 << "_";
						// 	arq << b << "," << psi_talb[t][a][l1][b] << "\n";
						// }
						if(l1 != data.bases[b]){
							...
							// 		arq <<con_amb_location[t][a][l1][b].get(GRB_DoubleAttr_Pi);
							// 		arq << "\n";
							// }
							theta_talb[t][a][l1][b] = con_amb_location[t][a][l1][b].get(
								GRB_DoubleAttr_Pi);
		...
		for(int c = 0; c < data.types_call; ++c){
			for(int l = 0; l < data.num_locals; ++l){
				// if(abs(phi_tcl[t][c][l] - con_queue[t][c][l].get(GRB_DoubleAttr_Pi)) > g_params.EPS){
				// 	arq << "phi_" << t+1 << "_" << c << "_" << l;
				// 	arq << "," <<  phi_tcl[t][c][l] << ",";
				// 	arq << con_queue[t][c][l].get(GRB_DoubleAttr_Pi) << "\n";
				// }
...
double CGCall::sub_problem(int t, int a, int l, int l1, int c, int h, int b){
	...
	int arr_time = get_time(data, (t+1)*data.quantum+data.tao[t][c][a][l1][l][h]);
	const int factor = 1;
		...
		for(int a = 0; a < data.types_amb; ++a){
			for(int l = 0; l < data.num_locals; ++l){
				...
				for(auto l1: closest_l1_to_l[l]){
					...
					int c = vc_tl[t][l];
					int b = vb_tal[t][a][l1];
					if(b != -1 && data.L_tab[t+1][a][b].find(l1) != data.L_tab[t+1][a][b].end()){
						double reduced_cost = sub_problem(t,a,l,l1,c,h,b);
						if(reduced_cost < -g_params.EPS){
							// std::cout << t+1 << " " << c << " " << a << " " << l1 << " " << b;
							// std::cout << " " << l << " " << h << ": " << reduced_cost << "\n";
							// std::cin.get();
							negative_found = true;
...
void CGCall::add_column(std::vector<int>& col){
	...
	int l = col[5], h = col[6];
	int t2 = get_time(data, (t+1)*data.quantum+data.tao[t+1][c][a][l1][l][h]);
	...
	if(data.L_tab[t+1][a][b].find(l1) !=  data.L_tab[t+1][a][b].end() &&
		...
		data.H[c][l].find(h) != data.H[c][l].end() && data.A[c].find(a) != data.A[c].end()){
		// con_base[t][a][b]
	...
	if(t2 < data.num_times && data.H[c][l].find(h) != data.H[c][l].end() &&
		...
		// con_hospital[t2][a][h]
		// std::cout << "1 " << con_hospital[t2][a][h].get(GRB_StringAttr_ConstrName) << "\n";
	...
	for(int l2 = 0; l2 < data.total_locals; ++l2){
		if(data.L_tab[t+1][a][b].find(l2) != data.L_tab[t+1][a][b].end() &&
			...
			// std::cout << "-1 " << con_location[t][a][l2][b].get(GRB_StringAttr_ConstrName) << "\n";
	...
	if(data.L_tab[t+1][a][b].find(l1) != data.L_tab[t+1][a][b].end() && 
		data.H[c][l].find(h) != data.H[c][l].end() && data.A[c].find(a) != data.A[c].end()){
		// std::cout << "1 " << con_amb_location[t][a][l1][b].get(GRB_StringAttr_ConstrName) << "\n";
		column.addTerm(1,con_amb_location[t][a][l1][b]);
	}
	// std::cout << "COLUMN: " << t << " " << c << " " << a << " " << l1 << " " << b << " ";
	// std::cout << l << " " << h << "\n";
	try{
		name << "xt" << t+1 << "_c" << c << "_a" << a << "_l'" << l1 << "_b" << b << "_l";
		name << l << "_h" << h;
		double f_val = f(t+1,c,a,l1,data.bases[b],l,h,data,1);
		xt_calblh[t][c][a][l1][b][l][h] =  model.addVar(0,GRB_INFINITY, f_val,GRB_CONTINUOUS,
			column, name.str());
...
void CGCall::add_bases_constraints_t0(){
	...
	for(int a = 0; a < data.types_amb; ++a){
		for(int b = 0; b < data.num_bases; ++b){
			...
			con_base_t0[a][b] = model.addConstr(con, GRB_EQUAL, 
				At_ab[0][a][b], name.str());
			name.str("");
			con = 0;
		}
	}
...
void CGCall::add_hospitals_constraints_t0(){
	...
	for(int a = 0; a < data.types_amb; ++a){
		for(int h = 0; h < data.num_hosps; ++h){
			...
			con_hospital_t0[a][h] = model.addConstr(con, GRB_EQUAL, 
				A0_ah[a][h], name.str());
			name.str("");
			con = 0;
		}
		...
		for(int b = 0; b < data.num_bases; ++b){
			for(int l = 0; l < data.total_locals; ++l){
				if(data.L_tab[0][a][b].find(l) != data.L_tab[0][a][b].end()){
					...
					con_location_t0[a][l][b] = model.addConstr(con,GRB_EQUAL, 
...
void CGCall::add_queues_constraints_t0(){
	...
	for(int c = 0; c < data.types_call; ++c){
		for(int l = 0; l < data.num_locals; ++l){
			...
			con_queue_t0[c][l] = model.addConstr(con, GRB_EQUAL, 
	...
	for(int t = 0; t < data.num_times-1; ++t){
		for(int a = 0; a < data.types_amb; ++a){
			for(int b = 0; b < data.num_bases; ++b){
				...
				try{
					con_base[t][a][b] = model.addConstr(con, GRB_EQUAL, 
	...
	for(int t = 0; t < data.num_times-1; ++t){
		for(int a = 0; a < data.types_amb; ++a){
			for(int h = 0; h < data.num_hosps; ++h){
				...
				for(int b = 0; b < data.num_bases; ++b){
					for(int l1 = 0; l1 < data.total_locals; ++l1){
						if((t)*quantum <= data.tao[0][type_call0][a][l1]
							[local_call0][h] && data.tao[0][type_call0][a][l1]
							[local_call0][h] <= (t+1)*quantum &&
							data.H[type_call0][local_call0].find(h) != 
							data.H[type_call0][local_call0].end() &&
							data.A[type_call0].find(a) != 
							data.A[type_call0].end() &&
				...
				con_hospital[t][a][h] = model.addConstr(con, GRB_EQUAL, 0, 
						name.str());
				name.str("");
				con = 0;
			}
				...
				for(int l1 = 0; l1 < data.total_locals; ++l1){
					if(data.L_tab[t+1][a][b].find(l1) !=
					 data.L_tab[t+1][a][b].end()){
						...
						con_location[t][a][l1][b] = model.addConstr(con,GRB_EQUAL, 
							At_alb[t+1][a][l1][b], name.str());
						name.str("");
						con = 0;
					}
				}
	...
	for(int t = 0; t < data.num_times-1; ++t){
		for(int c = 0; c < data.types_call; ++c){
			for(int l = 0; l < data.num_locals; ++l){
				...
				con_queue[t][c][l] =  model.addConstr(con,GRB_EQUAL,Ct_cl[t+1][c][l], name.str());
				name.str("");
				con = 0;
			}
		}
			...
			for(int b = 0; b < data.num_bases; ++b){
				for(int l1 = 0; l1 < data.total_locals; ++l1){
					if(data.L_tab[t+1][a][b].find(l1) != data.L_tab[t+1][a][b].end()){
						...
						name << b << "_l" << l1;
						con_amb_location[t][a][l1][b] = model.addConstr(con, GRB_LESS_EQUAL, At_alb[t][a][l1][b],
							name.str());
						name.str("");
						con = 0;
					}
	...
	for(int t = 0; t < data.num_times-1; ++t){
		for(int a = 0; a < data.types_amb; ++a){
			for(int b = 0; b < data.num_bases; ++b){
				...
				con_amb_base[t][a][b] = model.addConstr(con, GRB_LESS_EQUAL, 
					At_ab[t][a][b], name.str());
				name.str("");
				con = 0;
			}
		}
...
void CGCall::add_base_cap_constraints(){
	...
	for(int t = 0; t < data.num_times; ++t){
		for(int b = 0; b < data.num_bases; ++b){
			...
			con_cap_base[t][b] = model.addConstr(con,GRB_LESS_EQUAL,data.cap_bases[b], name.str());
			name.str("");
			con = 0;
		}
	}
...
// 	// Ambulance stopped
// 	if(amb.waiting_return || (!amb.busy && amb.base_ret == -1 && !amb.waiting_return)){ 
// 		return amb.location;
// 	}

// 	// Ambulance already at location
// 	if(amb.busy && amb.location == data.hospitals[amb.call_h_dest] ||
// 		!amb.busy && amb.base_ret != -1 && amb.location == data.bases[amb.base_ret]){
// 		return amb.location;
// 	}
...
// 	for(int b = 0; b < data.num_bases; ++b){
// 		if(amb.location == data.bases[b] && amb.base_ret == -1 && !amb.waiting_return){
// 			return b;
// 		}
// 	}
// 	return -1;
...
int CGCall::h_tall(int t, int a, int l1, int l){
	...
	for(auto h: data.H[0][l]){
		...
		int arrival_time = get_time(data, current_time+data.tao[t][0][a][l1][l][h]);
...
CGCall::~CGCall(){
	...
	delete[] A0_ab; delete[] A0_ah; delete[] A0_alb;
	...
	delete[] A0_callh; delete[] A0_calh; delete[] C;
	...
	delete[] beta_tab; delete[] alpha_tah; delete[] psi_talb; delete[] phi_tcl;
	delete[] ups_tb; delete[] theta_talb; delete[] gamma_tab;
...
CGAmbulance::CGAmbulance(Data & data, GRBEnv & env, Solver& solver):
	data(data), env(env), solver(solver), model(env), columns_added(0){
	std::stringstream name;
	...
	for(int a = 0; a < data.types_amb; ++a){
		...
		for(int h = 0; h < data.num_hosps; ++h){
			...
			for(int  b = 0; b < data.num_bases; ++b){
				name << "y0" << "_a" << a << "_h" << h << "_b" << b;
				y0_ahb[a][h][b] = model.addVar(0, A0_ah[a][h], 0, GRB_CONTINUOUS, name.str());
				name.str("");
			}
		}
	}
					...
					for(int l = 0; l <data.num_locals; ++l){
						...
						for(int h = 0; h < data.num_hosps; ++h){
							if(data.A[c].find(a) != data.A[c].end() &&
								...
								name <<"_l"<<l<<"_h" << h;
								xt_cablh[t][c][a][b][l][h] = model.addVar(0,
									data.num_ambs, 0, GRB_CONTINUOUS, name.str());
								name.str("");
							}
						}
					}
	...
	for(int t = 0; t < data.num_times-1; ++t){
		...
		for(int a = 0; a < data.types_amb; ++a){
			...
			for(int h = 0; h < data.num_hosps; ++h){
				...
				for(int b = 0; b < data.num_bases; ++b){
					name << "yt" << t+1 << "_a" << a << "_h" << h << "_b" << b; 
					yt_ahb[t][a][h][b] = model.addVar(0,data.num_ambs,0, GRB_CONTINUOUS, 
						name.str());
					name.str("");
				}
			}
	...
	for(int t = 0; t < data.num_times; ++t){
		...
		for(int c = 0; c < data.types_call; ++c){
			...
			for(int l = 0; l < data.num_locals; ++l){
				name << "Ct" << t+1 << "_c" << c << "_l" << l; 
				Ct_cl[t][c][l] = model.addVar(0, GRB_INFINITY, 0, GRB_CONTINUOUS, name.str());
				name.str("");
			}
		}
	}
	...
	for(int t = 0; t < data.num_times; ++t){
		...
		for(int a = 0; a < data.types_amb; ++a){
			...
			for(int b = 0; b < data.num_bases; ++b){
				name << "At" << t+1 << "_a" << a << "_b" << b;
				At_ab[t][a][b] = model.addVar(0,data.num_ambs, 0, GRB_CONTINUOUS, name.str());
				name.str("");
			}
		}
	}
		...
		for(int a = 0; a < data.types_amb; ++a){
			...
			for(int l = 0; l < data.total_locals; ++l){
				...
				for(int b = 0; b < data.num_bases; ++b){
					...
					At_alb[t][a][l][b] = model.addVar(0,data.num_ambs,0, GRB_CONTINUOUS, 
						name.str());
					name.str("");
				}
			}
...
void CGAmbulance::solve(){
	...
	try{
		...
		std::cout << model.get(GRB_IntAttr_Status) << " " << ex.getMessage() << "\n";
		model.computeIIS();
		for(int a = 0; a < data.types_amb; ++a){
			std::cout << a << " 10 3 " << A0_alb[a][10][3] << "\n";
			std::cout << a << " 3 " << A0_ab[a][3] << "\n";
			std::cout << "======\n"; 
		}
		model.write("amb_143.ilp");
		std::cin.get();
	}
	// std::cin.get();
	...
	for(int a = 0; a < data.types_amb && !found; ++a){
		for(int h = 0; h < data.num_hosps && !found; ++h){
			for(int b = 0; b < data.num_bases && !found; ++b){
				if(y0_ahb[a][h][b].get(GRB_DoubleAttr_X) > 0.5-g_params.EPS){
					amb_type = a;
					src_hosp = h;
					base_return = b;
					found = true;
			...
			for(int h1 = 0; h1 < data.num_hosps  && !found; ++h1){
				for(int l = 0; l < data.num_locals && !found; ++l){
					for(int h = 0; h < data.num_hosps && !found; ++h){
						if(data.A[c].find(a) != data.A[c].end() &&
							data.H[c][l].find(h) != data.H[c][l].end() &&
							y0_ahclh[a][h1][c][l][h].get(GRB_DoubleAttr_X) > 0.5-g_params.EPS){
							amb_type = a;
							call_type = c;
							call_location = l;
							src_hosp = h1;
...
double CGAmbulance::sub_problem(int t, int a, int l, int l1, int c, int h, int b){
	int next_l = data.L(t+1,a,l1,data.bases[b]);
	int arr_time = get_time(data, (t+1)*data.quantum+data.tao[t][c][a][l1][l][h]);
	const int factor = 1;
	double reduced_cost = f(t+1,c,a,l1,l,h,data,factor);
...
void CGAmbulance::add_column(std::vector<int> & col){
	...
	for(int l2 = 0; l2 < data.total_locals; ++l2){
		if(data.L_tab[t+1][a][b].find(l2) != data.L_tab[t+1][a][b].end() &&
			data.L(t,a,l1, data.bases[b]) == l2 && data.A[c].find(a)!=data.A[c].end() &&
			data.H[c][l].find(h) != data.H[c][l].end()){
			// con_location[t][a][l2][b]
			// std::cout << "-1 " << con_location[t][a][l2][b].get(GRB_StringAttr_ConstrName) << "\n";
			column.addTerm(-1,con_location[t][a][l2][b]);
