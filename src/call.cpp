...
Call::Call(): id(-1), time(-1), location(null_location), disc_time(-1), region(-1), hospital(-1),
	priority(-1), hosp_needed(false), clean_needed(false), cleaning(false), time_on_scene(-1), 
	time_at_hospital(-1), cleaning_time(-1), end(-1){}

Call::Call(int id, double time, Location& location, int hospital, int cleaning, 
		int priority, bool hosp_needed, bool clean_needed, double time_on_scene, 
		double time_at_hospital, double cleaning_time): id(id), time(time), 
		location(location), disc_time(-1), region(-1), hospital(hospital), cleaning(cleaning), 
		priority(priority), hosp_needed(hosp_needed), clean_needed(clean_needed), 
		time_on_scene(time_on_scene), time_at_hospital(time_at_hospital),
		cleaning_time(cleaning_time), end(-1){
}
