...
#include "../include/quick_generator.h"
#include "../include/quick_generator_regressor.h"
...
/*Cost functions for the model, the variables for the base b, and hospital h1 needs 
to be the real location, not its index (ex: bases[b]/hospitals[h1] instead of b/h1). 
*/
double f(int t, int c, int a, int b_h1, int l, int h, Data& data,
	int factor){
	// std::cout << t << " " << c << " " << a << " " << b_h1 << " ";
	// std::cout << l << " " << h << ", factor " << factor  << "\n";
	return (int)data.tao[t][c][a][b_h1][l][h]*factor;
}


double f(int t, int c, int a, int l1, int b, int l, int h,
	Data& data, int factor){
	return (int)data.tao[t][c][a][l1][l][h]*factor;
}
